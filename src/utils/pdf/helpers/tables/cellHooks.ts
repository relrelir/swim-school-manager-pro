...
export function willDrawCell(data: CellHookData): void {
  ...
  // For ID numbers, use explicit LTR EMBEDDING (stronger than just mark)
  if (/^\d{5,9}$/.test(cellContent)) {
    cell.text = [`\u202A${cellContent}\u202C`]; // LRE + PDF (Left-to-Right Embedding with Pop Directional Format)
  }
  // For phone numbers, use explicit LTR EMBEDDING
  else if (/^0\d{1,2}[\-\s]?\d{7,8}$/.test(cellContent)) {
    cell.text = [`\u202A${cellContent}\u202C`]; // LRE + PDF
  }
  // For numbers and numeric content, use explicit LTR EMBEDDING
  else if (/^[\d\.,]+$/.test(cellContent)) {
    cell.text = [`\u202A${cellContent}\u202C`]; // LRE + PDF
  }
  // For Hebrew text cells, use explicit RTL EMBEDDING
  else if (/[\u0590-\u05FF]/.test(cellContent)) {
    cell.text = [`\u202B${cellContent}\u202C`]; // RLE + PDF (Right-to-Left Embedding with Pop Directional Format)
  }
}
